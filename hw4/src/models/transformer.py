...
class TransformerRecommender(nn.Module):
    def __init__(self, config: myTransformerConfig, embed_user_gmf, embed_item_gmf, embed_user_mlp, embed_item_mlp):
        super().__init__()

        if config is None:
            config = myTransformerConfig()
        self.config = config
        self.embed_user_gmf = embed_user_gmf
        self.embed_item_gmf = embed_item_gmf
        self.embed_user_mlp = embed_user_mlp
        self.embed_item_mlp = embed_item_mlp
        self.layers = nn.Sequential(*[myTransformerBlock(config) for _ in range(config.n_layers)])
        self.projector = nn.Linear(self.config.hidden_size, 1, bias=False)

    def forward(self, users, items):
        users_gmf = self.embed_user_gmf(users)
        items_gmf = self.embed_item_gmf(items)
        users_mlp = self.embed_user_mlp(users)
        items_mlp = self.embed_item_mlp(items)
        input_sequence = [users_gmf, items_gmf, users_mlp, items_mlp]
        input_sequence = torch.stack(input_sequence, dim=1)  # (B, T, H)
        last_hidden_states = self.layers(input_sequence)
        features = torch.mean(last_hidden_states, dim=1)
        scores = self.projector(features).squeeze(dim=1)
        return scores
